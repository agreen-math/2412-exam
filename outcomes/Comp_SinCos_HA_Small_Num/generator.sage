...
class Generator(BaseGenerator):
    def data(self):
        ...
        # Use Pythagorean Triples to guarantee clean fractions and NO nested radicals!
        triples = [
            (3, 4, 5), (4, 3, 5), 
            (5, 12, 13), (12, 5, 13), 
            (8, 15, 17), (15, 8, 17), 
        ...
        # Apply half-angle identities and evaluate
        if outer == "sin":
            ...
            plug_tex = rf"{sign_str}\sqrt{{\frac{{1 + \left({format_math(cos_a)}\right)}}{{2}}}}"
        else: # tan (using the safe identity that doesn't require +/- analysis)
            ans = (1 - cos_a) / sin_a
        ...
        step4 = rf"\text{{Using the reference triangle, we identify: }} x = {x_val}, \quad y = {y_val}, \quad r = {r_val}"
