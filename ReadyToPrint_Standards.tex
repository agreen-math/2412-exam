...
\newpage
\question Suppose a rocket carrying fireworks is launched from a hill 88 feet above a lake. The rocket's height \(h\) (in feet) above the lake at time \(t\) (in seconds) is given by
...
\newpage
\uplevel{In problems \ref{eq_start} through \ref{eq_end}, solve for \textbf{all} solutions. Identify any extraneous solutions.}
\question[10] \label{eq_end} \(\displaystyle \frac{x}{x + 4} - \frac{20}{(x + 4)(x - 1)} = \frac{-11}{x - 1}\) 
