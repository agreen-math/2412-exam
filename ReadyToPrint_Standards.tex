...
\end{solution} \vspace{\stretch{3}}\vspace{12pt}
...
\newpage
\question Suppose a rocket carrying fireworks is launched from a hill 48 feet above a lake. The rocket's height \(h\) (in feet) above the lake at time \(t\) (in seconds) is given by
...
\(t = \frac{-96}{2(-16)} = \frac{-96}{-32}\) 
 ...
 \(\boxed{t = 3 \text{ sec}}\)
\end{solution}  \vspace{\stretch{1}} \vspace{12pt} \answerline
...
\question[10] \label{eq_start} \(x^{2} - 4 \, x + 7 = 0\) 
...
\newpage
\uplevel{In problems \ref{eq_start} through \ref{eq_end}, solve for \textbf{all} solutions. Identify any extraneous solutions.}
\question[10] \label{eq_end} \(\displaystyle \frac{x}{x + 3} - \frac{27}{(x + 3)(x - 6)} = \frac{-8}{x - 6}\) 
...
\end{solution} \vspace{\stretch{2}}\vspace{12pt}\answerline \newpage
